...
if(isset($_GET['id']) && $_GET['id']!=''){
	...
	if($check>0){
		...
		$image=$row['image'];
		$multipleimages=mysqli_query($con,"select id,product_images from product_images where product_id='$id' ");
		if(mysqli_num_rows($multipleimages)>0)
		{
			$jj=0;
			while ($rowmultipleimages=mysqli_fetch_assoc($multipleimages)) 
			{
				$multipleimagesarr[$jj]['product_images']=$rowmultipleimages['product_images'];
				$multipleimagesarr[$jj]['id']=$rowmultipleimages['id'];
				$jj++;
...
if(isset($_POST['submit'])){
	...
	$price2=get_safe_value($con,$_POST['price2']);
	$price3=get_safe_value($con,$_POST['price3']);
	$price4=get_safe_value($con,$_POST['price4']);
	...
	if($msg==''){
		if(isset($_GET['id']) && $_GET['id']!=''){
			if($_FILES['image']['name']!=''){
				...
				$update_sql="update product set categories_id='$categories_id',name='$name',price='$price',price2='$price2',price3='$price3',price4='$price4',qty='$qty',short_desc='$short_desc',description='$description',meta_title='$meta_title',meta_desc='$meta_desc',meta_keyword='$meta_keyword',image='$image',best_seller='$best_seller',sub_categories_id='$sub_categories_id' where id='$id'";
			}else{
			...
			$image=rand(111111111,999999999).'_'.$_FILES['image']['name'];
			move_uploaded_file($_FILES['image']['tmp_name'],PRODUCT_IMAGE_SERVER_PATH.$image);
			mysqli_query($con,"insert into product(categories_id,name,price,price2,price3,price4,qty,short_desc,description,meta_title,meta_desc,meta_keyword,status,image,best_seller,sub_categories_id) values('$categories_id','$name','$price','$price2','$price3','$price4','$qty','$short_desc','$description','$meta_title','$meta_desc','$meta_keyword',1,'$image','$best_seller','$sub_categories_id')");
			$id=mysqli_insert_id($con);
	
		}
                    //  product multi image upload start
		if(isset($_GET['id']) && $_GET['id']!='')
		{
			...
			foreach($_FILES['product_images']['name'] as $key=>$val)
			{
				if($_FILES['product_images']['name'][$key]!='')
				{
                       if(isset($_POST['product_images_id'][$key]))
					   {
						...
						$check=mysqli_query($con,"update product_images set product_images='$image' where id='".$_POST['product_images_id'][$key]."'");
					   ...
					   else
					   {
						$image=rand(111111111,999999999).'_'.$_FILES['product_images']['name'][$key];
						move_uploaded_file($_FILES['product_images']['tmp_name'][$key],PRODUCT_MULTIPLE_IMAGE_SERVER_PATH.$image);
						$check=mysqli_query($con,"insert into product_images (product_id,product_images) values('$id','$image')");
                     ...
                     <div class="card">
                        ...
                        <form method="post" enctype="multipart/form-data">
							...
							<div class="form-group">
								<div class="row">
                                   <div class="col-lg-2">
								   <label for="categories" class=" form-control-label">Price for 0.5 Kg Cake</label>
									<input type="text" name="price" placeholder="Enter product price" class="form-control" required value="<?php echo $price?>">
									</div>
                                   <div class="col-lg-2">
								   <label for="categories" class=" form-control-label">Price for 1 Kg Cake</label>
									<input type="text" name="price2" placeholder="Enter product price" class="form-control" required value="<?php echo $price?>"> 
									</div>
									<div class="col-lg-2">
									<label for="categories" class=" form-control-label">Price for 1.5 Kg Cake</label>
									<input type="text" name="price3" placeholder="Enter product price" class="form-control" required value="<?php echo $price?>">
									</div>
									<div class="col-lg-2">
									<label for="categories" class=" form-control-label">Price for 2 Kg Cake</label>
									<input type="text" name="price4" placeholder="Enter product price" class="form-control" required value="<?php echo $price?>">	
									</div>
